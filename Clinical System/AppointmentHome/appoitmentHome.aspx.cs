...
public partial class appoitmentHome : System.Web.UI.Page
{
    ...
    void getTimeClinic()
    {
        try
        {
            ...
            lblStartTime.Text = reader[0].ToString();
            lblCloseTime.Text = reader[1].ToString();
            ...
            for (int j = 0; j < 4; j++)
            {
                ...
                for (int k = 0; k < 7; k++)
                {
                    ...
                    if (k == 0 && i <= dateTime.Hour)
                    {
                        ...
                        todayCell.BorderColor = Color.LightGreen;
    ...
    public void refreshTable()
    {
        ...
        string maxFind = "SELECT cast(time_slot as time(0)) as time_slot, RTRIM(REPLACE(appt_date, ' 00:00:00', '')) AS appt_date FROM appt;";
        SqlCon.Open();
    ...
    public void CellChange(int hour, int min, int col)
    {
        string buttonID = $"btn_{hour}{min * 15}{col}";
    ...
    protected void BtnBookModal_Click(object sender, EventArgs e)
    {
        ...
        if (RbtnListRegistrationCheck.SelectedIndex == 0)
        {
            ...
            if (flag)
            {
                ...
                //change the button
                string id = Session["btnID"].ToString();
    ...
    //protected void RegNoValidation_ServerValidate(object source, ServerValidateEventArgs args)
    //{
    //    string str = args.Value;
    //    //args.IsValid = false;

    //    if (Convert.ToInt32(str) == 5)
    //    {
    //        args.IsValid = true;
    //    }
